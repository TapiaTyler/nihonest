...
export default async function GuidedJourneyPage({ params, searchParams }: PageProps<"/explore/journeys/[journeyId]">) {
  ...
  const steps = getJourneySteps(journey.id, selectedRoute?.id);
  const requiredStepNumbers = new Map(
    steps.filter(({ step }) => step.requiredness === "required").map(({ article }, index) => [article.metadata.id, index + 1]),
  );
  ...
  return (
    <div className="page-shell py-12 sm:py-20">
      ...
      {requestedRouteId && !selectedRoute && (
        <p className="mt-8 rounded-2xl border border-amber-200 bg-amber-50 p-5 text-amber-950">
          That route is not part of this journey. Choose one of the available routes below.
        </p>
      )}
      ...
      <nav className="mt-10 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm" aria-labelledby="journey-outline-heading">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <h2 id="journey-outline-heading" className="mt-2 text-2xl font-semibold tracking-tight text-slate-950">
              {selectedRoute ? `${selectedRoute.title} route` : "Your focused path"}
            </h2>
          </div>
          {selectedRoute && <Link href={journeyHref(journey.id)} className="inline-flex min-h-11 items-center rounded-full px-4 text-sm font-semibold text-teal-800 hover:bg-teal-50 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-teal-700">Change route</Link>}
        </div>
              ...
              {containsRouteChoice && !selectedRoute && (
                <div className="mt-6 grid gap-4 md:grid-cols-2">
                  {journey.routes.map((route) => (
                    <JourneyRouteLink key={route.id} href={journeyHref(journey.id, route.id)} journeyId={journey.id} routeId={route.id} focusedArticleId={route.articleId} className="group rounded-2xl border border-teal-200 bg-teal-50/50 p-6 hover:border-teal-500 hover:bg-teal-50 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-teal-700">
              ...
              {phaseSteps.length > 0 && (
                <div className="mt-6 grid gap-6 md:grid-cols-2">
                  {phaseSteps.map(({ article, step }) => {
                    return (
                      <div key={article.metadata.id} className={`relative h-full ${step.requiredness === "required" ? "pt-5" : ""}`}>
                        {step.requiredness === "required" ? (
                          <div className="pointer-events-none absolute left-5 top-0 z-10">
                            <span className="rounded-full bg-teal-800 px-3 py-1 text-xs font-semibold text-white">Step {requiredStepNumbers.get(article.metadata.id)}</span>
                          </div>
                        ) : null}
                        <ArticleCard
                          article={article.metadata}
                          href={articleJourneyHref(article.metadata.slug, journey.id, selectedRoute?.id)}
