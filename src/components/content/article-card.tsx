...
export function ArticleCard({ article, returnTo, href, notice }: ArticleCardProps) {
  ...
  return (
    <article className="h-full">
      <Link
        ...
        className="group flex h-full cursor-pointer flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition-[border-color,box-shadow,transform] hover:-translate-y-0.5 hover:border-teal-300 hover:shadow-md focus-visible:border-teal-400 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-teal-700 focus-visible:shadow-md"
      >
        {notice && (
          <p className="-mx-6 -mt-6 mb-5 border-b border-teal-200 bg-teal-50 px-6 py-3 text-sm font-semibold leading-6 text-teal-950">
            {notice}
          </p>
        )}
